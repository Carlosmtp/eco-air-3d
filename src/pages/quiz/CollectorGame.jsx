...
function randomPosition(range = 4) {
  return [(Math.random() - 0.5) * range, 0.1, (Math.random() - 0.5) * range];
}
...
const CollectorGame = () => {
        ...
        case "good":
          {
            ...
            if (environmentState < ENVIRONMENT_MAX) {
              setEnvironmentState((prev) =>
                Math.min(prev + 10, ENVIRONMENT_MAX)
              );
            }
          }
          break;
  ...
  const removeItem = (item) => {
    ...
    switch (item.type) {
      case "good":
        setGoodItems((current) => [
          ...current.filter((i) => i.id !== item.id),
          regenerateItem("good", "green"),
        ]);
        break;
      case "harmful":
        setHarmfulItems((current) => [
          ...current.filter((i) => i.id !== item.id),
          regenerateItem("harmful", "black"),
        ]);
        break;
      case "neutral":
        setNeutralItems((current) => [
          ...current.filter((i) => i.id !== item.id),
          regenerateItem("neutral", "gray"),
        ]);
        break;
      case "temporal":
        setTemporalItems((current) => [
          ...current.filter((i) => i.id !== item.id),
          regenerateItem("temporal", "blue"),
        ]);
        break;
      case "trap":
        setTrapItems((current) => [
          ...current.filter((i) => i.id !== item.id),
          regenerateItem("trap", "red"),
        ]);
        break;
      default:
        break;
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", position: "relative" }}>
      ...
      >
        {user ? (
          <h3>Puntuación: {user.puntuacion}</h3>
        ) : (
          <p>No estás logueado.</p>
        )}
        <h3>Tiempo restante: {timeLeft}s</h3>
        {doublePointsActive && (
          <p style={{ color: "yellow" }}>Doble de puntos activo!</p>
        )}
        {invertedControls && (
          <p style={{ color: "red" }}>Controles Invertidos!</p>
        )}
      </div>
      ...
      {showIntroModal && (
        ...
        >
          ...
          <p>
            Usa únicamente las flechas del teclado para mover el recolector
            (Nave).
          </p>
          <div
          ...
          >
            <p>
              ...
              ></span>
              <strong>Moléculas Verdes (Buenas):</strong> Como O2 (oxígeno),
              suman puntos, añaden tiempo y mejoran la capa de ozono.
            </p>
            ...
            <p>
              ...
              ></span>
              <strong>Moléculas Negras (Malas):</strong> Como CO2 (dióxido de
              carbono), restan puntos y tiempo.
            </p>
            ...
            <p>
              ...
              ></span>
              <strong>Moléculas Grises (Neutras):</strong> Como N2 (nitrógeno),
              no afectan tus puntos ni tu tiempo.
            </p>
            ...
            <p>
              ...
              ></span>
              <strong>Moléculas Azules (Temporales):</strong> Como H2O (agua),
              brindan efectos positivos temporales (ej. doble puntaje).
            </p>
            ...
            <p>
              ...
              ></span>
              <strong>Moléculas Rojas (Trampa):</strong> Como NOx (óxidos de
              nitrógeno), invierten tus controles temporalmente.
            </p>
          </div>
          <p>
            Evita salir del área delimitada. Un objeto central emite pulsos que
            te empujan.
          </p>
          <button
            onClick={() => {
